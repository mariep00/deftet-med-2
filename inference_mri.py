...
from parallel_mri import SimpleParallelWrapper
    ...
    def inference(self):
        ...
        with torch.no_grad():
            ...
            for data in self.dataloader_val:
                ...
                # Input data
                # ----------------------------------
                cat = data['synset'][0] if 'synset' in data else 'MRI'
                name = data['name'][0] if 'name' in data else 'sample'
                ...
                print('surface_point shape:', surface_point.shape)
                print('surface_point min:', surface_point.amin(dim=1))
                print('surface_point max:', surface_point.amax(dim=1))
                print('surface_point mean:', surface_point.mean(dim=1))

                print('sdf_point shape:', points.shape)
                print('sdf_point min:', points.amin(dim=1))
                print('sdf_point max:', points.amax(dim=1))
                print('sdf_point mean:', points.mean(dim=1))

                # Ground-truth mesh is optional.
                # Original mesh datasets provide verts/faces.
                # MRI inference usually does not.
                if 'verts' in data and 'faces' in data:
                    all_verts = [
                        v.to(self.config.device).unsqueeze(0).expand(self.device_count, -1, -1)
                        for v in data['verts']
                    ]
                    all_faces = [
                        v.to(self.config.device).unsqueeze(0).expand(self.device_count, -1, -1)
                ...
                if self.save:
                    ...
                    save_name = os.path.join(save_name, cat)
